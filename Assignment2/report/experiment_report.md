...
本实验完成了一个可训练、可恢复、可观测和可复现的 MNIST DDPM 工程。1.16M 参数的 MiniUNet 在 RTX 4050 Laptop GPU 上约 6 分钟完成 20 个 epoch，噪声预测损失稳定收敛，并能够生成多样的手写数字。采样实验表明，200 步在视觉质量和代理指标几乎不变的情况下比 1000 步快 4.98 倍；50 步可获得 19.28 倍加速，但畸形样本有所增加。因此，1000 步适合作为质量基准，200 步 ancestral sampling 更适合作为日常生成与现场展示配置。
